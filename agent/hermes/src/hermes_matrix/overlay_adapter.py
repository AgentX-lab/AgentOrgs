"""AgentOrgs policy overlay on hermes-agent's native Matrix adapter.

Subclasses hermes-agent's stock mautrix adapter and adds:
  * outbound ``m.mentions`` enrichment for full-MXID pings
  * separate DM / group allow-lists
  * history buffering for unmentioned group chatter
  * image downgrade when the active model has no vision capability

Transport stays in the upstream adapter.
"""
...
def _normalize_image_body(body: str) -> str:
    ...
    filename to Hermes causes the agent to treat the turn like a file lookup
    instead of using the cached image plus vision enrichment.
    """
...
def _describe_dropped_media(source_content: dict, fallback_body: str) -> str:
    """Return a short textual summary for a media event."""
...
class MatrixAdapter(_NativeMatrixAdapter):
    """Thin subclass that overlays AgentOrgs policy on hermes native Matrix."""
    ...
    def _wrap_send_message_event(self) -> None:
        ...
        if self._client is None:
            return
        if getattr(self._client, "_agentorgs_mentions_wrapped", False):
            return
        ...
        self._client._agentorgs_mentions_wrapped = True  # type: ignore[attr-defined]
    ...
    async def _resolve_message_context(
        ...
        """Apply allow/history policy around native mention gating."""
        ...
        if _is_commandish(body):
            # Slash commands do not receive a history prefix and
