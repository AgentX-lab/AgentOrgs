"""
Bridge: translate workspace openclaw.json into hermes-agent's
``~/.hermes/`` layout.
...
flows from openclaw.json) and leaves all other keys untouched.
...
Bridge-owned YAML blocks:
  ...
  ``logging.level`` (when ``AGENTORGS_MATRIX_DEBUG=1``),
...
def _port_remap(url: str, is_container: bool) -> str:
    """Remap container-internal :8080 to a host-published gateway port when needed."""
    if not is_container and url and ":8080" in url:
        gateway_port = os.environ.get("AGENTORGS_PORT_GATEWAY", "18080")
...
def _write_env_file(path: Path, env: Dict[str, str]) -> None:
    ...
    does not silently shadow a still-valid value from a previous bridge run.
    """
    lines = [
...
def _matrix_env(cfg: Dict[str, Any]) -> Dict[str, str]:
    ...
    require = matrix_raw.get("requireMention")
    if require is None:
        require = (matrix_raw.get("groups") or {}).get("*", {}).get(
            "requireMention", True
        )
    ...
    env: Dict[str, str] = {
        ...
        "MATRIX_REQUIRE_MENTION": "true" if require else "false",
...
def _model_env(cfg: Dict[str, Any]) -> Dict[str, str]:
    ...
    endpoint when ``model.provider`` is "custom".
    """
    m = _resolve_active_model(cfg)
    if not m:
        return {}
...
def _runtime_env(cfg: Dict[str, Any]) -> Dict[str, str]:
    ...
    ``vars`` itself) are recursed into one level deep.
    """
...
def _auxiliary_vision_yaml_block(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    model's runtime env. Point auxiliary vision at the same OpenAI-compatible
    endpoint and token as the active model.
    """
...
def _logging_yaml_block() -> Dict[str, Any]:
    """Raise Hermes log verbosity when Matrix debugging is enabled."""
    if os.environ.get("AGENTORGS_MATRIX_DEBUG") != "1":
        return {}
    return {"level": "DEBUG"}
...
def bridge_openclaw_to_hermes(
    ...
    Writes (idempotent across runs):
      ...
      ``<hermes_home>/config.yaml``
          ...
          ``logging.level: DEBUG`` (when ``AGENTORGS_MATRIX_DEBUG=1``), top-level
          ``matrix:`` block, and
